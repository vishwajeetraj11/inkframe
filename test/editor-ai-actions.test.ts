...
describe("AI editor actions parser", () => {
  it("extracts the action block and retires legacy preset aliases", () => {
    const parsed = parseAIEditorActionsFromMessage(`Use this direction.\n[[EDITOR_ACTIONS]]\n{"targetAspect":"active","scenes":[{"text":"Hello world","stylePreset":"kinetic grid","fontFamily":"script","textAlign":"left","animation":{"in":"punch","out":"fade","durationSeconds":0.4}}],"transitionSeconds":0}\n[[/EDITOR_ACTIONS]]`);
    ...
    expect(parsed.actions?.scenes[0]).toMatchObject({
      ...
      fontFamily: "sans",
      textAlign: "left",
      animation: { in: "punch", out: "fade", durationSeconds: 0.4 },
    });
  });
