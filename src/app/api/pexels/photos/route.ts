...
import { PexelsApiError, searchPexelsPhotos } from "@/lib/pexels/client";
...
const responseCache = new Map<string, { expiresAt: number; value: PexelsPhotoSearchResult }>();
...
const cachedSearch = async (
  ...
  const value = await searchPexelsPhotos(params, {
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    return jsonError(getErrorMessage(error, "Unexpected Pexels photo search error."), 500);
  }
}
